...
const perform = async (z, bundle) => {
  ...
  if (dedupe_key === undefined) {
    const [columns, error] = await query(host, user, port, tidbPassword,
        database, `select COLUMN_NAME,COLUMN_KEY
  ...
  // judge id
  const [id, error2] = await query(host, user, port, tidbPassword,
  ...
  operation: {
    ...
    inputFields: [
      ...
      },
      {
        ...
        label: 'Dedupe Key',
        dynamic: 'column.name.name',
        helpText: 'We default to what the primary key is on the table as the unique key to deduplicate on. If you prefer another field, select it here (invalid when the table has id column!).',
      },
    ],
